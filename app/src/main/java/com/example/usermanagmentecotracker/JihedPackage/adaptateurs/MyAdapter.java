...
// Bouton pour supprimer un élément via le DAO
        holder.button.setOnClickListener(v -> {
            int currentPosition = holder.getAdapterPosition();
            if (currentPosition != RecyclerView.NO_POSITION) {
                new Thread(() -> {
                    // Suppression dans la base de données via le DAO
                    luminositeDAO.deleteLuminosite(luminositeList.get(currentPosition));
                    ...
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(() -> {
                        notifyItemRemoved(currentPosition);
                        notifyItemRangeChanged(currentPosition, luminositeList.size());
                        Toast.makeText(context, "Élément supprimé avec succès", Toast.LENGTH_SHORT).show();
                    });
                }).start();
            } else {
                Toast.makeText(context, "Erreur : Impossible de supprimer l'élément", Toast.LENGTH_SHORT).show();
            }
        });
